...
| `material` | `'glossy'` | One of the fifteen [material presets](materials-and-theming.md#material-presets), or `{ preset, ...overrides }` with material properties plus `outline`, `layer`, `shader`, `thicknessScale`, `attenuationScale`, `environmentScale`, and `surfaceSide`. |
