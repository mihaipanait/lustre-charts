...
export const MATERIAL_CONTROLS = {
  ...
  tricolor: [
    range('Color harmony', 'shader.dominance', 'Palette dominance', 0.05, 1, 0.01, 0.2, 'Share retained across the full gradient; the palette color still anchors its broad center band.'),
    range('Color harmony', 'shader.edgePower', 'Rim contrast', 0.35, 4, 0.05, 1.35, 'Controls the extra companion-color lift on surfaces viewed at a grazing angle.'),
    range('Color harmony', 'shader.flow', 'Gradient direction', 0.25, 3, 0.05, 1.15, 'Turns and ripples the broad three-color gradient across each object.'),
    optics.transmission(0.48),
    ...opticalSurface(),
    optics.volume(),
    optics.attenuation(),
    optics.ior(1.4),
    surface.roughness(0.16, 0.2),
    surface.clearcoat(0.72),
    surface.coatRoughness(0.11),
  ],
  velvet: [
